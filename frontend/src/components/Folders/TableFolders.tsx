...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/folders/foldersSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureFoldersCols';
...
const TableSampleFolders = ({
  ...
  useEffect(() => {
    if (foldersNotify.showNotification) {
      notify(foldersNotify.typeNotification, foldersNotify.textNotification);
    }
  }, [foldersNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/folders/folders-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/folders/folders-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
    ...
    <>
      ...
      {!showGrid && (
        <BigCalendar
          events={folders}
          showField={'name'}
          start-data-key={'created_at'}
          end-data-key={'updated_at'}
          handleViewAction={handleViewAction}
          handleEditAction={handleEditAction}
          handleDeleteAction={handleDeleteModalAction}
          pathEdit={`/folders/folders-edit/?id=`}
          pathView={`/folders/folders-view/?id=`}
...
export default TableSampleFolders;
